...
Use **12 RunComfy image and video models** inside ComfyUI, with live Model API pricing and your own RunComfy API Token. Each model has its own node and one native `IMAGE` or `VIDEO` output.
...
All names below have the `RunComfy` prefix in the node search. Video nodes appear under `RunComfy/Video`, image nodes under `RunComfy/Image`.
...
Open **Settings → RunComfy → Account → API Token → Configure account** to enter your RunComfy API Token from [RunComfy Profile](https://www.runcomfy.com/profile). This account is shared by all 12 nodes. The dialog sends it to your ComfyUI server, validates it using a read-only model request, and stores it in an ignored `runcomfy_config.json` file with owner-only permissions.
...
1. Import the corresponding example from the table above.
2. Select your files in `Load Image` / `Load Video` where present, and enter a prompt. `example.png` and `reference.mp4` are placeholder filenames; the examples do not bundle input media.
...
5. Choose **fixed** to reuse the result while editing downstream nodes, or **randomize** to request a new generation on each run. Queue the workflow. `Save Video` preserves embedded audio; `Save Image` writes native image results to the output folder.
...
- Reference nodes accept an `images` batch of up to 9 images, three `VIDEO` ports and three `AUDIO` ports. The 4K version requires at least one reference video. Use ComfyUI's native Load Video and Load Audio nodes.
...
**A new generation is paid; reusing a cached result is not another submission.** Fixed inputs and a fixed seed reuse ComfyUI's cached output when available, so changing only a downstream resize or save node does not force another paid generation. Wan exposes its provider seed; the other models use `generation_seed` only to control reruns, without promising deterministic provider output. Changing the account invalidates cached generations. ComfyUI's cache is not durable across restarts; use an existing request ID to recover a result after restarting.
...
The mappings cover all twelve RunComfy targets across Seedance, Wan, FLUX, Nano Banana, and Seedream. This does not mean every official configuration has an equivalent: unsupported connected outputs, reference limits, or settings must be resolved before switching. Seedance 2.5 4K is an explicit resolution-changing choice because the compared official implementation has no matching 2.5/4K setting. Similar model names do not guarantee identical provider revisions or output quality.
...
- **Variable pricing:** Reference-to-Video, Wan, FLUX and Seedream display `Base $…/s` or `Base $…/image`. The current catalog exposes a base price and prose notes, but no structured parameter-specific quote. Resolution, audio and reference-video duration can change billing, so these nodes do not display a per-run total. `Base` is not a guaranteed minimum. Exact parameter-sensitive previews require a backend quote endpoint; no rate multipliers are hardcoded here.
...
See [the switching and runtime validation report](docs/validation-2026-09-22.md) for current checks and limitations, including an upstream SaveVideo Undo/Redo edge. The models have not all had individual paid end-to-end generation tests.
